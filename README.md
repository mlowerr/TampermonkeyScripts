...
- **cPanel Email Filters Helper.user.js**: Adds a helper bar on cPanel email filters pages to export existing rules, jump to the bottom of the page for saving, and open a form-based multi-rule builder (header/operator/interaction/action) that stays open after adding a set so more filter types can be entered before closing. It defaults new rules to `From` when available, normalizes bracketed email input for sender/recipient headers (including `To` and `Any Recipient`), and inserts fresh rule rows before writing each value.
